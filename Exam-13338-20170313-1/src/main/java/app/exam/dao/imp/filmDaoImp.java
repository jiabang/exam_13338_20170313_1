...
public class filmDaoImp implements filmDao {
	...
	public List<filmEntity> film_allQuery() throws Exception {
		String sql = "SELECT film_id,title,description,name " + "FROM film f ,language g "
	...
	@Override
	public int film_add(filmEntity film) throws Exception {
	...
	public int film_upt(filmEntity file_upt) throws Exception {
		...
		ps.setString(2, file_upt.getDescription());
		ps.setInt(3, file_upt.getLanguage_id());
		ps.setInt(4, file_upt.getFilm_id());
		int i = ps.executeUpdate();
		return i;
	}
